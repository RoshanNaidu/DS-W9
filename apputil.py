...
class GroupEstimate(object): # A model that predicts using group-wise mean or median
    ...
    def fit(self, X, y): # X: DataFrame of features, y: Series of target variable
    ...
    def predict(self, X_list): # X_list: DataFrame of features for prediction
        preds, missing = [], 0
        for row in X_list:                      
            key = tuple(row)
            if key in self.group_values.index: # Check if group exists
        ...
        return np.array(preds)  # Ensure output is a numpy array
